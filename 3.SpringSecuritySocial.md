...
Spring Security默认提供当前账户绑定信息的服务,服务的请求地址为`/conncet`,访问的方式的`POST`
...
//将用户信息放进model
	...
	//返回视图名字:/connect/status
	//spring默认没有提供这个视图，需要进行自定义
...
方法中获取了当前账户所有的,并且已经在系统中配置了的第三方应用的绑定信息,并且存入Model中.

**自定义回显视图`connect/status`**
...
3. 定义`AbstractConnectionViewProcessor`借口默认实现类
...
在社交总配置`SocialConfig.java`代码中,已经贴出了相关配置:
...
Spring Social已经实现了绑定与解绑的逻辑,提供了`/connect/providerId`服务,根据请求方法的不同,执行不同的业务处理,POST方式进行绑定,DELETE方式进行解绑,与绑定信息的获取同属与同一个类中,`providerId`代表系统已经配置好的服务提供商的id
...
public class ConnectController implements InitializingBean {
    ...
    //获得授权码,根据授权码获取token,用户信息,最后返回一个Connection
    @RequestMapping(
        value = {"/{providerId}"},
        method = {RequestMethod.GET},
        params = {"code"}
...
从上面列中源码中绑定的两个方法,则是走OAuth协议,与之前的实现QQ,微信登录流程是一样的,绑定成功与解绑成功后会跳转指定的视图
...
而且,sping social并未提供该视图,因此需要自定义
...
`WeixinAutoConfiguration.java`中增加配置
